...
window.encounterUi = (() => {
  ...
  const relationship = value => ({ with: 'TEAMMATE', against: 'OPPONENT', 'same-match': 'SAME MATCH' }[value] || 'SAME MATCH');
  const playlist = value => ({ competitive: 'Competitive', unrated: 'Unrated', swiftplay: 'Swiftplay', spikerush: 'Spike Rush', deathmatch: 'Deathmatch', hurm: 'Team Deathmatch', ggteam: 'Escalation' }[value] || value || 'Unknown mode');
  let view = null;
  ...
  function badge(player) {
    ...
    const parts = [h.with ? `${num(h.with)} with` : '', h.against ? `${num(h.against)} against` : '', h.sameMatch ? `${num(h.sameMatch)} same match` : ''].filter(Boolean);
    return `<span class="encounter-badge" title="Open the player profile to see your saved shared matches">PLAYED BEFORE · ${esc(parts.join(' · '))}</span>`;
  }
  ...
  function open(player, live, api, showProfile) {
    ...
    $('#playerProfileContent').innerHTML = `<div class="player-profile-hero encounter-hero"><div><p class="eyebrow">${player.side === 'enemy' ? 'OPPONENT' : 'TEAMMATE'} PROFILE</p><h1 id="playerProfileTitle" class="encounter-name">${esc(player.name)}</h1><p>${esc(player.agent)} · Account level ${num(player.level)}</p><p>${esc(player.rank)}${player.peakRank ? ` · Peak ${esc(player.peakRank)}` : ''}</p></div></div><div class="player-profile-body"><div class="player-profile-section-title"><p class="eyebrow">PLAYED TOGETHER</p><h2>Shared match history</h2></div><div class="encounter-counts"><span><strong>${num(active.history.with)}</strong> as teammates</span><span><strong>${num(active.history.against)}</strong> as opponents</span>${active.history.sameMatch ? `<span><strong>${num(active.history.sameMatch)}</strong> in the same match</span>` : ''}</div><p class="encounter-scope">Matches saved by BYAKUGAN across Acts and game modes. Older games without saved roster data may be missing.</p><p id="encounterWarning" class="encounter-scope" role="status">${esc(active.history.warning)}</p><div id="encounterMatches"></div><p id="encounterPageCount" class="encounter-scope"></p><button id="encounterMore" class="ghost-button" hidden>Load more matches</button></div>`;
    const draw = () => {
      $('#encounterMatches').innerHTML = active.history.matches.map(row).join('') || '<div class="empty-state">No shared matches were found in your saved history.</div>';
      $('#encounterPageCount').textContent = `${active.history.matches.length} of ${active.history.total} saved shared matches`;
