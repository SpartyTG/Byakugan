...
function renderSenseiPanel(entry, selector = '#senseiWorkspacePanel') {
  ...
  if (status === 'ready' && entry.report) {
    ...
    const chat = (entry.chat || []).map((message) => `<div class="sensei-chat-message ${escapeHtml(message.role)}"><strong>${message.role === 'assistant' ? 'SENSEI' : 'YOU'}</strong><br>${escapeHtml(message.text)}</div>`).join('');
        const noticeText = String(entry.notice || '');
    ...
    const fallbackNotice = isLiteFallback && cleanedNotice ? `<div class="sensei-notice"><strong>LOCAL MODEL FALLBACK</strong><span>${escapeHtml(cleanedNotice)}</span></div>` : '';
        ...
        const mission = entry.brain && entry.brain.title ? `<div class="sensei-mission-card"><small>${missionClosed ? 'MISSION CLOSED' : entry.brain.keptOpenMission ? 'MISSION CONTINUES' : 'THIS MATCH MISSION'}</small><h3>${escapeHtml(entry.brain.title)}</h3><p>${escapeHtml(entry.brain.why || '')}</p><p><strong>Drill:</strong> ${escapeHtml(entry.brain.drillName || '')} — ${escapeHtml(entry.brain.drillSetup || '')}</p><p><strong>Done when:</strong> ${escapeHtml(entry.brain.successMetric || '')}</p>${missionClosed ? `<p>Closed as ${escapeHtml(entry.brain.status === 'resolved_by_user' ? 'done' : entry.brain.status)}.</p>` : '<p class="sensei-mission-help"><strong>Keep</strong> if this card still matches how you died or played. Example: it says stop dying first on attack, and you died first on attack again. The same mission stays up next game.<br><br><strong>Wrong</strong> if the card is talking about the wrong thing. Example: it blames first deaths, but you were traded, AFK, or the deaths were on defense. Sensei will hide that lesson for a while.<br><br><strong>Done</strong> if you already did the drill, or that problem stopped. Example: you ran the custom routes and you are not dying first anymore. Sensei can pick a new lesson later.</p><div class="sensei-mission-actions"><button type="button" class="ghost-button" data-sensei-mission="keep" title="Stay on this lesson">Keep</button><button type="button" class="ghost-button" data-sensei-mission="wrong" title="This is not my leak">Wrong</button><button type="button" class="ghost-button" data-sensei-mission="done" title="I finished this drill">Done</button></div>'}</div>` : '';
