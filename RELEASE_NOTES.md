...
- Imports only the visible Matches, compact W/L record, K/D Ratio, and Headshot %, derives draws from the verified result total, and caches the result per Riot account and Act.
...
- Recognizes Tracker's actual compact record badges such as `20 W` and `19 L`, then verifies that wins, losses, and derived draws reconcile exactly with Matches Played.
