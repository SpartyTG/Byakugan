...
`BYAKUGAN-CURRENT-HANDOFF.md`, and this package targets `v0.8.0-beta.131`.

## Verified baseline

- Previous canonical version: `v0.8.0-beta.130`
- Automated tests: `194 passed, 0 failed`, plus `10` Sensei Brain smoke checks
- Syntax validation: passed
- Repository: `https://github.com/SpartyTG/Byakugan`

## Beta.131 authoritative full-Act recovery

Beta.131 merges Riot's general match history with the separately paginated
competitive-update index, reindexes already-reduced schema-6 caches, and does
not treat short pages as the end of an Act. Recovery waits until VALORANT is
outside Agent Select and active matches, then hydrates at bounded concurrency.

## Beta.130 in-app patch notes
...
Beta.129 keeps a complete same-Act dataset visible while newer matches hydrate
and always unions a partial rescan with every previously cached match. Existing
schema-5 cache files stay usable but trigger one authoritative background
reindex, allowing an incorrectly reduced 44/44 cache to recover without another
temporary collapse. Legitimate account and Act boundaries remain isolated.
...
Install beta.131 on both PCs and follow the manual verification in
