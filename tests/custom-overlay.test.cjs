...
test('custom Reactive Vision supplies separately editable bars and switches them by game state', () => {
  assert.ok(CUSTOM_ELEMENT_TYPES.includes('reactiveBetween'));
  assert.ok(CUSTOM_ELEMENT_TYPES.includes('reactiveInGame'));
  ...
  assert.match(overlayScript, /element\.id === 'reactiveBetween' && compact/);
  assert.match(overlayScript, /element\.id === 'reactiveInGame' && !compact/);
