...
## Included in version 0.8.0-beta.28
...
- Redesigned `560 × 220` Awakened Rank card with denser spacing and larger, stream-legible rank, peak, session, and match text
- Live background-opacity control from fully transparent at 0% to completely solid at 100%
- Reliable agent footer that shows the live selected agent or clearly labeled last-played agent while in menus
- Dedicated high-contrast last-match text panel separated from the animated RR beam track
- Full-width RR beam track with the last-match RR stacked directly above the victory/defeat result on the far right
...
`release` folder. The resulting `BYAKUGAN-Setup-0.8.0-beta.28-x64.exe` installs
...
exact application version prefixed by `v`—for example `v0.8.0-beta.28`. Upload
...
example `0.8.0-beta.28`, before rebuilding and uploading all three artifacts.
...
```bash
git tag v0.8.0-beta.28
git push origin v0.8.0-beta.28
```
