...
- Duplicate completed matches are retained; placeholders may be replaced. Imports validate account and Act, skip ambiguous early ties, and do not invent historical RR.
- Upload henrik-history-import.json from the PC initiating the import to inspect combined totals and reconciliation.
- Persistent streaming snapshot caching, gameplay Act scanning, and automatic Act diagnostic transfer from beta.139–142 are included.


- Overview dynamically reads every signed-in user's current-Act matches and presents Win, Loss, Draw, K/D, headshot percentage, and current RR
- Riot's misleading 155-match `Total` value no longer stops pagination; BYAKUGAN continues until the actual Act boundary
- Six 20-match index pages load concurrently, while corrected response cursors prevent short pages from creating gaps
- Draws contribute to Act combat totals without being classified as losses
- Match-detail hydration retries temporary throttling, timeout, and Riot server failures at a safer concurrency of eight
- Cache schema 12 forces one corrected reindex while preserving the monotonic same-Act archive

### Included from beta.132 and beta.131

- Full-Act recovery unions Riot's general match-history index with the separately paginated competitive-update index, recovering older same-Act match IDs when either source is shortened
- Existing schema-6 caches are reindexed once, repairing the incorrect 44/44 dataset that beta.129 could mistakenly preserve as complete
- Short Riot pages advance from the number actually received; only an empty page or explicit previous-Act boundary proves the competitive index is complete
- Full-Act scans wait until VALORANT is outside Agent Select and active matches, with match details hydrated at a bounded concurrency of four
...
- Existing act-cache files receive a one-time background reindex while their current data stays visible, allowing beta.129 to recover caches that an earlier build incorrectly reduced

- Riot reads are now request-smoothed: menus poll every 15 seconds while Agent Select and active matches retain 5-second responsiveness, overlapping live and full refreshes coalesce, failures back off, and post-match history waits for Riot Client to settle before refreshing
- Completed-history peak enrichment is limited to ten previously uncached players per refresh with a concurrency of two, then fills progressively; successful peak summaries persist locally for 24 hours so restarting BYAKUGAN does not recreate the original cold-cache burst

- Resolved rank, RR, peak rank, account level, rank art, and overlay/session rank now remain stable when an individual Riot profile request temporarily fails; a successful response can still apply legitimate placement, Act-reset, or account changes
