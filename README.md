...
## Included in version 0.8.0-beta.54
...
- Reorganized Reactive Vision between-games dock with last-match result, RR movement, W/L, and K/D grouped in one enlarged summary; current RR placed beside Current Rank so the all-time peak row sits fully above the footer branding; substantially larger Session Performance labels and values; and responsive in-game rank sizing that prevents truncation on narrow OBS sources
...
`release` folder. The resulting `BYAKUGAN-Setup-0.8.0-beta.54-x64.exe` installs
...
exact application version prefixed by `v`—for example `v0.8.0-beta.54`. Upload
...
example `0.8.0-beta.54`, before rebuilding and uploading all three artifacts.
...
```bash
git tag v0.8.0-beta.54
git push origin v0.8.0-beta.54
```
