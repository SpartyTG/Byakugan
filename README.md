...
## Included in version 0.8.0-beta.50
...
- Reorganized Reactive Vision between-games dock with last-match result, RR movement, W/L, and K/D grouped in one enlarged summary; equal-size current and all-time peak ranks stacked as aligned top and bottom rows; and BYAKUGAN Session Vision branding moved into the footer
...
`release` folder. The resulting `BYAKUGAN-Setup-0.8.0-beta.50-x64.exe` installs
...
exact application version prefixed by `v`—for example `v0.8.0-beta.50`. Upload
...
example `0.8.0-beta.50`, before rebuilding and uploading all three artifacts.
...
```bash
git tag v0.8.0-beta.50
git push origin v0.8.0-beta.50
```
